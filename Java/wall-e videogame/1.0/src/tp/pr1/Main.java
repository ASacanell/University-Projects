...
public class Main {
	...
	public Main(){	}
		...
		/**
		 * Metodo que crea los lugares del mapa
		 * @return Array con los lugares
		 */
	private static Place[] createPlaces(){
		...
		/**
		 * Metodo que genera las uniones entre los lugares del mapa (las calles)
		 * @param _places Array con los lugares del mapa
		 * @return	Crea un array de calles, que unen los lugares
		 */
	...
	public static void main(String[] args) {
		...
		Place[] places = createPlaces();	//Inicializa los lugares del mapa
		Street[] citymap = createStreets(places);	//Inicializa las calles del mapa
		Place initialPlace = places[0];		//Estable el lugar 0 como lugar inicial
		Direction direccion = Direction.NORTH;	//Establece la direccion Norte como direccion inicial
		
		RobotEngine robot = new RobotEngine(initialPlace, direccion, citymap);	//Crea una instancia de robotEngine, que sera nuestro Wall-E
		
		if(!robot.initGame(initialPlace))
		{
