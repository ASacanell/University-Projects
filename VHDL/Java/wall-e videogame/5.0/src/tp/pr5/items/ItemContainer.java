...
public class ItemContainer extends Observable<InventoryObserver>{
	...
	public void useItem(Item extItem)
	{
		if(!extItem.canBeUsed()){
			...
			for(InventoryObserver reo: observadores){
				reo.itemEmpty(extItem.getId());
			}
			this.notifyInventoryUpdate(this);
			
		}
	}
		
	public Item getIt(int i){
		return this.ItemList.get(i);
	}
	
	//Set the ArrayList of PlayerObservers.
		public ArrayList<InventoryObserver> getObservadores(){
			return this.observadores;
		}

		public void notifyInventoryUpdate(ItemContainer robotContainer) {
			
			ArrayList <Item> inventory= new ArrayList<Item>();
			
			for(int i = 0; i < robotContainer.numberOfItems(); i++){
				robotContainer.orderList();
				//String[] item = {robotContainer.getIt(i).getId(), robotContainer.getIt(i).getDescription()};
				inventory.add(robotContainer.getIt(i));
			}
			
			for(InventoryObserver reo: observadores){
				reo.inventoryChange(inventory);
			}
			
		}

		public void notifyItemScanned(String item) {
			for(InventoryObserver reo: observadores){
				reo.itemScanned(item);
			}
			
		}

		public void notifyInventoryScanned(String inventory){			
			if(inventory==null)
			{
				for(InventoryObserver reo: observadores){				
					reo.inventoryScanned(null);
				}
			}
			else{
				for(InventoryObserver reo: observadores){				
					reo.inventoryScanned(inventory);
				}
			}
		}
